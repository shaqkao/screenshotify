...
//
// Vendored from tauri-winrt-notification 0.7.3 and patched to add
// `Toast::launch`, which sets the `launch` attribute on the root <toast>
// element. Without it, WinRT only activates the app when a button is
// clicked — tapping the toast body itself does nothing. Upstream has no
// public API for this (checked as of 0.8.1), so it's patched in locally
// rather than pinned on a fork. See create_template() and the `launch`
// field/method for the change; everything else is unmodified upstream
// source (test module removed, it depended on unshipped resource files).
...
impl Toast {
    ...
    /// Sets the `launch` attribute on the toast, which is delivered as the
    /// `on_activated` argument when the user clicks the toast body itself
    /// (as opposed to one of its buttons). Without this, WinRT does not
    /// treat the toast body as activatable at all — only its buttons are.
    pub fn launch(mut self, args: &str) -> Toast {
        self.launch = format!(r#"launch="{}""#, xml_escape::escape(args));
        self
    }
